...
### C4. 重启后总是开新局（存档被覆盖 / 弹出"放弃游戏"确认框）

**现象**：重启游戏后没有恢复存档，而是从第一层开新局；偶尔弹出"你确定吗？放弃游戏会视为一次失败"确认框，一旦选中"是"就毁掉存档。

**原因**：`CLICK LEFT x y` 用的是**逻辑分辨率坐标（1920×1080）**，与窗口实际大小无关。旧值 `(240,660)` 映射到的是"放弃当前游戏"按钮（"继续"下面一格），所以每次点到的都是"放弃"→ 弹确认框 → 新开一局覆盖存档。这个问题会导致反复从第一层重打、进度全丢。

**解决**：实测窗口几何（客户区 1280×720，桌面原点 (323,206)，缩放 0.6667）后用系统鼠标点击验证，正确坐标为逻辑 `(205,631)`；并加"确认框选否"的兜底交替点击（`DIALOG_NO_CLICK`，没有确认框时该点落在 logo 上，无害）。

**以后改坐标的验证方法**：
```powershell
# 1) 量窗口：GetWindowRect / GetClientRect / ClientToScreen → 得到客户区原点与缩放
# 2) 桌面坐标 = 客户区原点 + 逻辑坐标 × 缩放，用 SetCursorPos + mouse_event 实测点击
# 3) 截图确认界面变化（或看 driver_log.txt 里紧接着的状态）
```
...
| 攒了 400 金币从不消费、进商店就离开 | 商店房间的 `shop` 选项从没被点开，驱动直接 `PROCEED` 走人 | 进商店房间先选 `shop` 进店（每层一次）；并把 GRID/奖励屏的分支放在"房间=商店"兜底判断**之前**（否则删卡 GRID、战斗奖励屏都会落到商店逻辑里死循环）|
| 战斗中点地图 → 游戏状态损坏、永久卡死 | CommunicationMod 在战斗抽牌动画中会上报 `MAP` 界面；此时点地图节点会破坏状态 | 只有"场上没有存活怪物"时才允许点地图节点；怪物还活着就把状态路由回出牌 |
| 删除/转化卡牌界面无限循环 | ① 删卡 GRID 打开时 `room_type` 仍是商店，被商店分支抢走；② 多选界面（星盘转化 3 张）被当成单选，反复点同一张来回切换 | 界面类型判断优先于房间类型；GRID 读 `num_cards`/`selected_cards`，按优先级选**未选过**的卡，选满后 `CONFIRM` |
| 静默"扫腿"等带目标技能反复被拒（死循环）| 格挡/departure 路径用 `PLAY n` 不带目标，游戏报 `Selected card requires an enemy target`，驱动反复重发同一张 | 统一 `play_card()`：`has_target` 的卡自动附带最大威胁的目标索引，无目标卡不带 |
| 观战者被自己的愤怒姿态打死 | 影子姿态表漏了 `Vigilance`（初始牌里唯一的退出姿态手段），驱动以为永远处于愤怒（双倍受伤）；低血时还会主动进愤怒 | 补全姿态表；愤怒下危险回合优先退出姿态；血量 <55% 且来袭 ≥12 时过滤掉"进愤怒"的牌（除非当回合能斩杀）|
| 机械师"防御堆满、打不死 Boss"（240 血 Boss 打到 1 血输掉）| 选卡门槛被"稀释保护"抬到 2.8，攻击牌也被挡在门外 | 攻击牌用门槛 -0.6 的宽松线；攻击卡评级上调；牌组少于 4 张实质攻击时攻击牌 +0.6 |
