...
| **四职业支持** | 铁甲战士 / 静默猎手 / 机械师 / 观战者，各带卡牌数据、流派逻辑与职业专属出牌策略；`character.txt` 随时切换（含随机/轮换模式）|
...
- 四职业均已适配；其中**铁甲战士、静默猎手、机械师**已验证通过第一幕 Boss（史莱姆王/守卫者/六火亡魂）
- 机械师单局最深记录：**第三幕 floor 39**（连续通过第一、二幕 Boss）
- 普通战斗近乎零失血（平均净失血 -0.4 HP/场），死亡集中在第二幕厚血怪与 Boss 战
...
│   ├── character.txt             职业选择（IRONCLAD/THE_SILENT/DEFECT/WATCHER/RANDOM/ROTATE）
│   ├── mode.txt                  续档或开新局（continue/new）
│   └── rotate_count.txt          ROTATE 轮换计数（自动生成，跨重启持久化）
├── mods/
...
`driver/character.txt` 决定机器人给哪个角色开新局：

| 值 | 含义 |
|---|---|
| `IRONCLAD` | 铁甲战士 |
| `THE_SILENT`（或 `SILENT`）| 静默猎手 |
| `DEFECT` | 机械师 |
| `WATCHER` | 观战者 |
| `RANDOM` | 每局从可用职业里随机 |
| `ROTATE` | 每局轮换（铁甲 → 静默 → 机械 → 观战循环）|

说明：
- **只影响新开局的职业**：改完重启游戏生效；已经进行的存档不受影响（续档仍是原职业）
- **未解锁的职业会自动跳过**：`START` 命令连续失败 3 次即判定该职业未解锁，随机/轮换时排除掉，并且每 8 局重试一次（因为解锁可能在运行中发生）
- **各职业专属逻辑**：
  - 铁甲战士：力量流（燃心/观察弱点/恶魔形态 × 多段攻击）、格挡流（金属化/壁垒/全身撞击）
  - 静默猎手：毒流（催化剂/尸爆）、幻影刃流、敏捷流；带目标技能（如扫腿）自动附带目标索引
  - 机械师：充能球体系——冰球被动格挡计入防御估值，`Zap`/`Dualcast` 在安全回合主动铺设；选卡偏向伤害牌（防止"防御堆满打不死 Boss"）
  - 观战者：姿态体系——愤怒（双倍伤害，双层结算）由驱动的影子追踪器维护（Mod 不上报姿态），危险时优先退出姿态；低血时不主动进愤怒；封印破除只在斩杀回合使用
